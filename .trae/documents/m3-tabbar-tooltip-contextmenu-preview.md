...
* `demo/assets/i18n/en-US.json`、`zh-CN.json`：新增 3 个 key，如果i18n里没有对应的key，则回退到硬编码的英文
...
| `demo/assets/i18n/en-US.json`、`zh-CN.json`      | i18n 资源     | M3'.1, M3'.3               |
| `demo/src/cases/tab_preview_case.rml`           | demo 集成     | M3'.6                      |
| `demo/src/cases/mod.rs`                         | case 注册     | M3'.6                      |

## 验证

### 单元测试

* `cargo test -p rust-rml-engine` — setters 单测覆盖所有新 event setter

* `cargo test -p rust-rml-ui` — Tab/TabBar API 单测

* `is_double_click` 纯函数单测（250ms 边界）

### 集成验证

1. `cargo check -p rust-rml-ui && cargo check -p rust-rml-engine && cargo check -p demo`
2. 启动 demo：

   * hover 关闭按钮 → tooltip 显示

   * 右键 tab → 弹出 Close/Close All/Close Others + 业务扩展项

   * 点击 Close → tab 关闭

   * 切换 en/zh → 菜单文案变化

   * preview tab 显示 italic

   * 双击 preview tab → 触发 promote 回调

### 风险项

* TabBar render 内部生成 PopupMenu 的 Entity 生命周期：需在闭包内创建并返回，避免借用问题

* 业务 `on_context_menu` 接收 `&mut PopupMenu`：需确认 PopupMenu 暴露足够 API（item/separator/submenu）

* 双击检测与 `on_click` 共存：确认单击不会误触发 promote
