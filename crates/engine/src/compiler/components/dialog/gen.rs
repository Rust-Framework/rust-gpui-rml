...
//! ## 受控模式 `open={field}`
//!
//! 当 `open` bind 属性存在时，进入受控模式：
//! - 不渲染 trigger（受控模式由 ViewModel 状态驱动显示/隐藏）
//! - 生成条件渲染：`if self.field { Dialog...into_any_element() } else { Empty }`
        ...
        match child {
                ...
                if e.slot_name.as_deref() == Some("trigger") && open_field.is_none() =>
            {
                ...
                if trigger_code.is_some() {
                    return Err(CodegenError {
                        message: "Dialog requires exactly one trigger slot (multiple found)"
                            .into(),
                        span: Some(elem.span),
                    });
                }
...
mod tests {
    ...
    #[test]
    fn gen_dialog_controlled_open_basic() {
        let elem = make_element(
            "Dialog",
            vec![
                make_bind_attr("open", "show_dialog"),
                Attribute::Static {
                    name: "title".into(),
                    value: "受控对话框".into(),
                    span: Span::empty(),
        ...
        let code = gen_dialog(&elem, None, 0, &ctx(), &mut 1, &Vec::new(), &[]).unwrap();
        // 条件渲染包装
        assert!(code.contains("if self.show_dialog {"));
        assert!(code.contains("gpui::Empty.into_any_element()"));
        // Dialog 仍然生成
        assert!(code.contains("rml_ui::Dialog::new(cx)"));
        assert!(code.contains(".title(\"受控对话框\")"));
        // 自动回写 on_close
        assert!(code.contains(".on_close(cx.listener("));
        assert!(code.contains("this.show_dialog = false"));
        assert!(code.contains("this.__rml_bump_version(\"show_dialog\")"));
        // 无 trigger
        assert!(!code.contains(".trigger("));
    }

    #[test]
    fn gen_dialog_controlled_open_with_content() {
        let elem = make_element(
            "Dialog",
    ...
    #[test]
    fn gen_dialog_controlled_open_with_user_on_close() {
        let elem = make_element(
            "Dialog",
            vec![
    ...
    #[test]
    fn gen_dialog_controlled_open_ignores_trigger() {
        let elem = make_element(
            "Dialog",
    ...
    #[test]
    fn gen_dialog_controlled_open_with_footer_slot() {
        let mut footer_btn = make_trigger();
        footer_btn.slot_name = Some("footer".into());
        let elem = make_element(
            "Dialog",
