...
/// 静态属性 → builder 方法
///
/// - `underline` / `pill` / `flat` / `outline` / `segmented` 布尔属性 → `.<name>()`（仅 Tabs）
...
pub fn static_setter(name: &str, value: &str, tag: &str) -> Option<String> {
    match name {
        // variant 布尔属性: underline → .underline() 等（仅 Tabs 支持，TabItem 无这些方法）
        "flat" | "outline" | "pill" | "segmented" | "underline" if tag == "Tabs" => {
...
mod tests {
    ...
    #[test]
    fn static_setter_tabs_variant_false_returns_none() {
        // variant 布尔属性值为 "false" 时不生成调用
        assert!(static_setter("underline", "false", "Tabs").is_none());
    }

    #[test]
    fn static_setter_tab_variants_not_supported() {
        // variant 布尔属性仅 Tabs 支持（Tab 底层 TabItem 无这些方法）
        assert!(static_setter("underline", "", "Tab").is_none());
        assert!(static_setter("pill", "", "Tab").is_none());
    }
