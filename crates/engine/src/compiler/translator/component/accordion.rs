//! Accordion 容器组件 translator
//!
//! 薄包装 `compiler::accordion::gen_accordion`，构造 + 属性 + AccordionItem 子节点注入。
...
#[derive(Debug)]
pub struct AccordionTranslator;

impl IRmlTranslator for AccordionTranslator {
    fn tag(&self) -> &'static str {
        "Accordion"
    }

    fn matches(&self, elem: &Element) -> bool {
        tags::canonical_tag(&elem.tag) == "Accordion"
    }
    ...
    fn to_rust(
        ...
        *id_counter += 1;
        ...
        if let Some(sheet) = &ctx.stylesheet {
            let style_code = apply_css_styles(elem, "Accordion", sheet, parents);
    ...
    fn metadata(&self) -> TranslatorMetadata {
        TranslatorMetadata::new("Accordion", "Accordion", ComponentCategory::Container).container(true)
    }
}

pub fn register(registry: &mut crate::compiler::translator::TranslatorRegistry) {
    registry.register(AccordionTranslator);
}
