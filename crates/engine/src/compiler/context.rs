//! 编译器上下文类型与编译入口
//!
//! 包含代码生成上下文（`CodegenCtx`）、校验规则、错误类型与 `compile()` 入口函数。
//! 从 `mod.rs` 抽离以满足「mod.rs 仅做模块声明与 re-export」约束。
...
pub enum CompileError {
    ...
    Validate(super::validator::ValidationError),
...
impl From<super::validator::ValidationError> for CompileError {
    fn from(e: super::validator::ValidationError) -> Self {
...
pub fn compile(source: &str, ctx: &CodegenCtx) -> Result<CompileOutput, CompileError> {
    ...
    super::validator::validate(&root, &ctx.registry, &ctx.user_components)?;
