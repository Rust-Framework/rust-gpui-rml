...
//! use rml_ui_term::{ColorPalette, TerminalConfig, TerminalView};
//!
...
use crate::input::{is_paste_keystroke, keystroke_to_bytes, normalize_paste_bytes};
use crate::layout::TerminalLayout;
use crate::pty::spawn_terminal;
use crate::scroll::TerminalScrollHandle;
use crate::mouse::{
    encode_modifiers, mouse_button_report, pixels_to_scroll_lines, scroll_report,
...
/// ```ignore
/// use gpui::{Edges, px};
/// use rml_ui_term::{ColorPalette, TerminalConfig};
///
...
/// });
/// ```
pub type BellCallback = Box<dyn Fn(&mut Window, &mut Context<TerminalView>) + Send + Sync>;
...
/// });
/// ```
pub type TitleCallback = Box<dyn Fn(&mut Window, &mut Context<TerminalView>, &str) + Send + Sync>;
...
/// });
/// ```
pub type ClipboardStoreCallback = Box<dyn Fn(&mut Window, &mut Context<TerminalView>, &str) + Send + Sync>;
...
/// });
/// ```
pub type ExitCallback = Box<dyn Fn(&mut Window, &mut Context<TerminalView>) + Send + Sync>;
...
pub struct TerminalView {
    ...
    /// Callback for terminal bell events
    bell_callback: Option<Arc<BellCallback>>,

    /// Callback for terminal title changes
    title_callback: Option<Arc<TitleCallback>>,

    /// Callback for clipboard store requests
    clipboard_store_callback: Option<Arc<ClipboardStoreCallback>>,

    /// Callback for terminal exit events
    exit_callback: Option<Arc<ExitCallback>>,

    /// PTY bytes waiting on the UI thread (filled by a blocking reader thread)
    output_pending: Arc<Mutex<VecDeque<Vec<u8>>>>,

    /// Signaled when `output_pending` has new data (consumed by the async wake task)
    wake_rx: Option<flume::Receiver<()>>,

    /// Terminal events received while flushing PTY responses (processed in `process_events`)
    deferred_events: VecDeque<TerminalEvent>,

    /// Geometry from the last paint pass (for hit-testing).
    layout: Arc<Mutex<TerminalLayout>>,

    /// gpui-component scrollbar bridge for scrollback history.
    scroll_handle: TerminalScrollHandle,

    /// When true, new PTY output keeps the viewport pinned to the live end.
...
impl TerminalView {
        ...
        R: Read + Send + 'static,
    {
        ...
        let output_pending = Arc::new(Mutex::new(VecDeque::new()));
        let output_for_thread = output_pending.clone();
        let exit_tx_for_thread = exit_event_tx.clone();
        let (wake_tx, wake_rx) = flume::unbounded::<()>();
        let auto_follow = Arc::new(AtomicBool::new(true));
        let scroll_handle =
            TerminalScrollHandle::new(state.term_arc(), wake_tx.clone(), auto_follow.clone());

        thread::spawn(move || {
            Self::read_stdout_blocking(stdout_reader, output_for_thread, exit_tx_for_thread, wake_tx);
        });
        ...
        Self {
            ...
            layout: Arc::new(Mutex::new(TerminalLayout::default())),
            scroll_handle,
            auto_follow,
            selecting: false,
        }
    ...
    fn follow_output_if_enabled(&mut self) {
        if self.auto_follow.load(Ordering::Relaxed) {
            self.state.scroll_to_bottom();
        }
    }

    fn prepare_for_user_input(&mut self) {
        self.auto_follow.store(true, Ordering::Relaxed);
        self.state.scroll_to_cursor();
    }

    /// Start the async task that applies PTY output on the UI thread when data arrives.
    pub fn start_pty_reader(&mut self, cx: &mut Context<Self>) {
        ...
        let wake_rx = self
            .wake_rx
            .take()
            .expect("start_pty_reader called more than once");

        self._reader_task = cx.spawn(async move |this: WeakEntity<Self>, cx: &mut AsyncApp| {
            loop {
                match wake_rx.recv_async().await {
                    Ok(()) => {
                        let result = this.update(cx, |view: &mut Self, cx: &mut Context<Self>| {
                            if view.drain_pending_output() {
                                cx.notify();
                            }
                        });
                        if result.is_err() {
    ...
    fn drain_pending_output(&mut self) -> bool {
        let chunks: Vec<Vec<u8>> = self.output_pending.lock().drain(..).collect();
        if chunks.is_empty() {
            return false;
        }
        for chunk in chunks {
            self.state.process_bytes(&chunk);
            self.flush_pty_writes_from_events();
        }
        self.follow_output_if_enabled();
        true
    }

    /// Write alacritty `PtyWrite` responses back to the PTY (required for PowerShell/cmd prompts).
    ///
    /// Uses `try_iter()` to drain `PtyWrite` events from the `mpsc` channel without blocking.
    /// This is called synchronously during `drain_pending_output()` on the UI thread. The
    /// non-blocking pattern is acceptable here because:
    /// - `PtyWrite` events are sparse (cursor queries, mode responses) and won't accumulate
    /// - `try_iter()` returns immediately when the channel is empty
    /// - PTY output processing is already batched per frame, so event latency is bounded
    fn flush_pty_writes_from_events(&mut self) {
        loop {
            let events: Vec<TerminalEvent> = self.event_rx.try_iter().collect();
            if events.is_empty() {
                break;
            }
            let mut had_pty_write = false;
            for event in events {
                match event {
                    TerminalEvent::PtyWrite(text) => {
                        had_pty_write = true;
                        self.write_pty_input(text.as_bytes());
                    }
    ...
    pub fn with_bell_callback(
        mut self,
        callback: impl Fn(&mut Window, &mut Context<TerminalView>) + Send + Sync + 'static,
    ) -> Self {
        self.bell_callback = Some(Arc::new(Box::new(callback)));
        self
    }
    ...
    pub fn with_title_callback(
        mut self,
        callback: impl Fn(&mut Window, &mut Context<TerminalView>, &str) + Send + Sync + 'static,
    ) -> Self {
        self.title_callback = Some(Arc::new(Box::new(callback)));
        self
    }
    ...
    pub fn with_clipboard_store_callback(
        ...
        self.clipboard_store_callback = Some(Arc::new(Box::new(callback)));
        self
    }
    ...
    pub fn with_exit_callback(
        ...
        self.exit_callback = Some(Arc::new(Box::new(callback)));
        self
    }
    ...
    fn read_stdout_blocking<R: Read + Send + 'static>(
        mut stdout_reader: R,
        output_pending: Arc<Mutex<VecDeque<Vec<u8>>>>,
        exit_event_tx: mpsc::Sender<TerminalEvent>,
        ...
        loop {
            match stdout_reader.read(&mut buffer) {
                Ok(0) => {
                    if exit_event_tx.send(TerminalEvent::Exit).is_err() {
                        tracing::warn!("Failed to send exit event (receiver dropped)");
                    }
                    if wake_tx.send(()).is_err() {
                        tracing::warn!("Failed to send wake signal (receiver dropped)");
                    }
                    break;
                }
                Ok(n) => {
                    output_pending.lock().push_back(buffer[..n].to_vec());
                    if wake_tx.send(()).is_err() {
                        tracing::warn!("Failed to send wake signal (receiver dropped)");
                    }
                }
                Err(e) => {
                    eprintln!("gpui-terminal: PTY stdout read error: {e}");
                    if exit_event_tx.send(TerminalEvent::Exit).is_err() {
                        tracing::warn!("Failed to send exit event (receiver dropped)");
                    }
                    if wake_tx.send(()).is_err() {
    ...
    fn write_pty_input(&self, bytes: &[u8]) {
        if bytes.is_empty() {
            return;
        }
        let mut writer = self.stdin_writer.lock();
        if let Err(e) = writer.write_all(bytes) {
            tracing::error!("Failed to write to PTY stdin: {e}");
        }
        if let Err(e) = writer.flush() {
            tracing::error!("Failed to flush PTY stdin: {e}");
        }
    }
    ...
    fn paste_from_clipboard(&mut self) -> bool {
        ...
        let Ok(text) = clipboard.paste() else {
            return false;
        };
        if text.is_empty() {
            return true;
        ...
        self.write_pty_input(&normalize_paste_bytes(&text));
        true
    }
    ...
    fn forwards_mouse_to_pty(&self) -> bool {
    ...
    fn alacritty_selection_type(click_count: usize) -> SelectionType {
        match selection_type_from_clicks(click_count) {
            crate::mouse::SelectionType::Simple => SelectionType::Simple,
            crate::mouse::SelectionType::Word => SelectionType::Semantic,
            crate::mouse::SelectionType::Line => SelectionType::Lines,
        }
    }
    ...
    fn on_mouse_down(
        ...
        let Some(viewport) = layout.pixel_to_viewport(event.position) else {
            return;
        };

        let display_offset = self.state.with_term(|term| term.grid().display_offset());
        let grid_point = layout.viewport_to_grid(display_offset, viewport);

        if self.forwards_mouse_to_pty() {
            let mode = self.state.mode();
            ...
            if let Some(bytes) =
                mouse_button_report(event.button, true, grid_point, mods, mode)
            {
                self.write_pty_input(&bytes);
            }
            return;
        ...
        if event.button == MouseButton::Left {
            self.selecting = true;
            let sel_type = Self::alacritty_selection_type(event.click_count);
            self.state.with_term_mut(|term| {
                term.selection = Some(Selection::new(sel_type, grid_point, Side::Left));
            });
            cx.notify();
        }
    }
    ...
    fn on_mouse_up(
        ...
        if self.forwards_mouse_to_pty() {
            ...
            if let Some(bytes) =
                mouse_button_report(event.button, false, grid_point, mods, mode)
            {
                self.write_pty_input(&bytes);
            }
        }
    ...
    fn on_scroll(
        ...
        let pixel_delta = event.delta.pixel_delta(layout.cell_height).y;
        // GPUI: positive Y = wheel up → older scrollback (positive Scroll::Delta).
        let lines = pixels_to_scroll_lines(pixel_delta, layout.cell_height);
        ...
        let display_offset = self.state.with_term(|term| term.grid().display_offset());
        let viewport = layout
            .pixel_to_viewport(event.position)
            .unwrap_or(alacritty_terminal::index::Point::new(
                alacritty_terminal::index::Line(0),
                alacritty_terminal::index::Column(0),
            ));
        let grid_point = layout.viewport_to_grid(display_offset, viewport);

        if let Some(bytes) = scroll_report(lines, grid_point, 0, mode) {
            self.write_pty_input(&bytes);
            return;
        }
        ...
        self.state.scroll_viewport_by(lines);

        if lines < 0 {
            self.auto_follow
                .store(self.state.is_at_bottom(), Ordering::Relaxed);
        }
    ...
    ///
    /// Drains deferred events (accumulated during PTY output processing) and then polls
    /// the `mpsc` channel with `try_recv()` for newly arrived events. Called from
    /// [`render`](Self::render) on every frame.
    ///
    /// This uses a non-blocking poll pattern rather than a background task because:
    /// - Terminal events (Bell, Title, Clipboard, Exit) are infrequent and lightweight
    /// - `try_recv()` returns immediately without blocking the render loop
    /// - Processing events in `render()` ensures they are handled before painting,
    ///   avoiding frame-delayed UI updates (e.g., title changes, exit transitions)
    /// - An async background task would introduce ordering complexity with the existing
    ///   push-based PTY output pipeline (`wake_rx` + `drain_pending_output`)
    fn process_events(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        while let Some(event) = self.deferred_events.pop_front() {
            self.handle_terminal_event(event, window, cx);
        }
...
impl Render for TerminalView {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        // Drain PTY output without cx.notify() — the wake task notifies when new data arrives.
        // Events (Bell, Title, Exit, etc.) are polled non-blocking via try_recv() in
        // process_events(); this is acceptable because events are infrequent and lightweight.
        let _ = self.drain_pending_output();
        ...
        let layout_cache = self.layout.clone();
        // Theme `selection` (#007acc) — much higher contrast than list_active on dark terminals.
        let selection_bg = cx.theme().selection.opacity(0.62);
        let scroll_handle_paint = self.scroll_handle.clone();
        let scroll_handle_bar = self.scroll_handle.clone();
        ...
        div()
            ...
            .child(
                canvas(
                    ...
                    move |bounds, _, window, cx| {
                        ...
                        use crate::layout::SCROLLBAR_WIDTH;
                        ...
                        let scrollbar_reserve = if history_size > 0 {
                            SCROLLBAR_WIDTH
                        } else {
                            px(0.0)
                        };
                        ...
                        scroll_handle_paint.set_cell_height(measured_renderer.cell_height);

                        let mut layout = layout_cache.lock();
            ...
            .child(
                div()
                    ...
                    .bottom_0()
                    .child(
                        Scrollbar::new(&scroll_handle_bar)
