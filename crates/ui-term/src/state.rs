...
//! use rml_ui_term::event::GpuiEventProxy;
//! use rml_ui_term::terminal::TerminalState;
//!
//! let (tx, rx) = channel();
...
impl TerminalState {
    ...
    /// `true` when the viewport shows the live end of the buffer (`display_offset == 0`).
    pub fn is_at_bottom(&self) -> bool {
        self.with_term(|term| term.grid().display_offset() == 0)
    }

    /// Jump to the newest output (live end).
    pub fn scroll_to_bottom(&self) {
        self.with_term_mut(|term| term.scroll_display(Scroll::Bottom));
    }
    ...
    /// Page the scrollback viewport by `lines` (positive = older, negative = newer).
    pub fn scroll_viewport_by(&self, lines: i32) {
        if lines == 0 {
            return;
        }
        self.with_term_mut(|term| term.scroll_display(Scroll::Delta(lines)));
    }
...
mod tests {
    ...
    #[test]
    fn test_scroll_to_bottom_and_viewport() {
        let (tx, _rx) = channel();
        let event_proxy = GpuiEventProxy::new(tx);
        let mut terminal = TerminalState::new(80, 24, event_proxy);
