...
//! | `Event::PtyWrite(_)` | `PtyWrite(String)` | Response bytes for the PTY |
...
//! `PtyWrite` must be written to the PTY or shells like PowerShell will hang.
//!
//! # Example
//!
//! ```
//! use std::sync::mpsc::channel;
//! use rml_ui_term::event::{GpuiEventProxy, TerminalEvent};
//!
//! let (tx, rx) = channel();
...
pub enum TerminalEvent {
    ...
    /// Bytes that must be written back to the PTY (cursor queries, mode responses, etc.).
    PtyWrite(String),
}
