...
//! ```ignore
//! use gpui::px;
//! use rml_ui_term::{ColorPalette, TerminalRenderer};
//!
...
use crate::layout::{SCROLLBAR_WIDTH, TerminalLayout};
...
impl TerminalRenderer {
    ...
    /// ```
    /// use gpui::px;
    /// use rml_ui_term::render::TerminalRenderer;
    /// use rml_ui_term::ColorPalette;
    ///
    ...
    pub fn paint(
        ...
        let scrollbar_width = if history_size > 0 {
            SCROLLBAR_WIDTH
        } else {
            px(0.0)
        };
        ...
        let content_width =
            bounds.size.width - padding.left - padding.right - scrollbar_width;
        ...
        *layout_out = TerminalLayout {
        ...
        for line_idx in 0..num_lines {
            ...
            let mut selection_rect: Option<BackgroundRect> = None;
            if let Some(ref range) = selection_range {
                if range.start.line <= grid_line && range.end.line >= grid_line {
                    let start_col = if range.start.line == grid_line {
                        range.start.column.0
                    } else {
                        0
                    };
                    let end_col = if range.end.line == grid_line {
                        range.end.column.0 + 1
                    } else {
                        num_cols
                    };
                    if start_col < end_col {
                        selection_rect = Some(BackgroundRect {
                            start_col,
                            end_col,
                            row: line_idx,
                            color: selection_bg,
            ...
            if let Some(sel) = selection_rect {
                let x = origin.x + self.cell_width * (sel.start_col as f32);
                let y = origin.y + self.cell_height * (sel.row as f32);
                let width = self.cell_width * ((sel.end_col - sel.start_col) as f32);
                let height = self.cell_height;
        ...
        if let Some(viewport_point) = point_to_viewport(display_offset, cursor_point) {
            if viewport_point.line < num_lines {
                let cursor_x = origin.x + self.cell_width * (viewport_point.column.0 as f32);
                let cursor_y = origin.y + self.cell_height * (viewport_point.line as f32);
