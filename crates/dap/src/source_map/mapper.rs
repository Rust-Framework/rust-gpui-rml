...
//! ## MVP 约束
//!
//! 当前 `rust-rml-engine` 的 codegen 不输出 `.rml` 行号 → 生成 `.rs` 行号的源映射，
//! 因此 `FilePairMapper` 仅做文件级配对（`.rml` ↔ `.rml.rs`），不翻译行号。
//! 精确行级映射需 engine codegen 增强输出 `.rml.map`（另立任务），届时实现
//! `LineAccurateMapper` 即可，上层代码无需改动（依赖 `SourceMapper` trait）。
...
pub trait SourceMapper: Send + Sync {
    ...
    fn rml_to_rust(&self, rml_uri: &Url, line: u32) -> Option<(Url, u32)>;

    /// 反向映射：`.rml.rs` 位置 → `.rml` 位置（供 UI）
    fn rust_to_rml(&self, rust_uri: &Url, line: u32) -> Option<(Url, u32)>;
}
...
impl SourceMapper for FilePairMapper {
    fn rml_to_rust(&self, rml_uri: &Url, line: u32) -> Option<(Url, u32)> {
        ...
        Some((rust_uri.clone(), line))
    }

    fn rust_to_rml(&self, rust_uri: &Url, line: u32) -> Option<(Url, u32)> {
        let rml_uri = self.rust_to_rml.get(rust_uri)?;
        Some((rml_uri.clone(), line))
    }
}
...
mod tests {
    ...
    fn forward_mapping_returns_paired_file() {
        ...
        let (mapped_uri, line) = mapper.rml_to_rust(&rml, 42).unwrap();
    ...
    fn reverse_mapping_returns_paired_file() {
        ...
        let rust = Url::parse("file:///foo/counter.rml.rs").unwrap();
        mapper.register_pair(rml.clone(), rust.clone());
        let (mapped_uri, line) = mapper.rust_to_rml(&rust, 10).unwrap();
    ...
    fn unregistered_file_returns_none() {
        ...
        assert!(mapper.rml_to_rust(&unknown, 1).is_none());
        assert!(mapper.rust_to_rml(&unknown, 1).is_none());
    }
}
