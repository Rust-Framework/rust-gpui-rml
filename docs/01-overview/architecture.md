...
> 💡 **如何阅读这张思维导图**：根节点是 RML 架构整体，三大主干分支对应三层。表现层是开发者直接编辑的文件；编译层在 `build.rs` 阶段对开发者透明；框架层是运行时支撑。箭头方向（编译层）表示表现层的 `.rml` 经编译层生成 GPUI 代码后，最终落在框架层运行。
